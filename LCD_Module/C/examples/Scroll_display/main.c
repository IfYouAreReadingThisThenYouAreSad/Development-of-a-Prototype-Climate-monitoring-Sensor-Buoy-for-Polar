...
int main(void)
{
  ...
  set_cursor(0, 0);
  send_string("Welcome to LCD1602.");  // Show a welcome message on the first row

  // Set the cursor to column 0, row 1 and display text on the second row
  set_cursor(0, 1);
  send_string("Let your imagination run wild.");  // Show another message on the second row
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	    ...
	    for (int i = 0; i < 14; i++) {
	        scroll_display_left();  // Scroll the text one step to the left
	        HAL_Delay(500);  // Wait for 500ms before scrolling again
	    }

	    HAL_Delay(1000);  // Wait for 1 second before the next action

	    // Scroll the text on the display to the right 14 times
	    for (int i = 0; i < 14; i++) {
	        scroll_display_right();  // Scroll the text one step to the right
	        HAL_Delay(500);  // Wait for 500ms before scrolling again
	    }

	    HAL_Delay(1000);  // Wait for 1 second before starting the loop again
