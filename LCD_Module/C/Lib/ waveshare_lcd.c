...
static I2C_HandleTypeDef *lcd_i2c_assigment;
...
static LCD_STATUS lcd_i2c_write_command(uint8_t function_byte) { // used for sending commands
...
static LCD_STATUS lcd_i2c_write_data(uint8_t function_byte) { // used for sending character
    uint8_t sending_buffer[2];
...
static LCD_STATUS i2c_write_backlight(uint8_t reg, uint8_t val) {
    uint8_t buffer[2];
    buffer[0] = reg;
    buffer[1] = val;
...
static LCD_STATUS backlight_init(void) {
...
static LCD_STATUS lcd_init(void) {
    HAL_Delay(100);
...
LCD_STATUS lcd_startup(I2C_HandleTypeDef *i2c_pin_assigment){
	...
	LCD_STATUS upload_status;
	upload_status = backlight_init();

	if(upload_status != LCD_OK) return LCD_ERROR_BACKLIGHT_INIT;


	upload_status = lcd_init();

	if(upload_status != LCD_OK) return LCD_ERROR_LCD_INIT;
