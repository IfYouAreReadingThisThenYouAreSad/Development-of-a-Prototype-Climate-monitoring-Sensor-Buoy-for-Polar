...
int main(void)
{
  ...
  cursor();  // Enable cursor on the LCD
  ...
  set_cursor(0, 0);
  left_to_right();  // Set text direction to left-to-right

  // Display the text "Waveshare" on the first row
  for (uint8_t i = 0; i < 9; i++) {  // Loop through the "Waveshare" text
      data(text[i]);  // Send character to LCD
      HAL_Delay(200);  // Wait for 0.2 seconds before displaying next character
  }

  right_to_left();  // Change text direction to right-to-left for scrolling
  ...
  for (int i = 11; i >= 0; i--) {  // Loop through the "Hello World!" text (backwards)
      if (i < 2) {
          ...
          set_cursor(x, 0);  // Set cursor to the updated X position
