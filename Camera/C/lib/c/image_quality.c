/*
 * image_quality.c
 *
 *  Created on: 26 Feb 2026
 *      Author: thomasneedham
 */


#include "image_quality.h"










//-----------------FUNCTIONS---------------------//





	//---- functions that process the greyscale image



static uint8_t mean(const JpegInformation *jpeg){

	uint32_t x = 0;


	for(uint32_t n = 0; n < jpeg->size; n++) x+= jpeg->data[n];

	uint8_t x_bar = x/jpeg->size;

	return x_bar;





}

static float variance(const JpegInformation *jpeg, const uint8_t mean){

	int32_t sum = 0;



	for(uint32_t i = 0; i < jpeg->size; i++){

		int32_t diff = (int32_t)jpeg->data[i] - (int32_t)mean;
		sum += diff*diff;



	}


	float var = (float)sum/(float)jpeg->size;
	return var;
}

static uint32_t saturation_high(const JpegInformation *jpeg){

	uint32_t saturation_high_temp = 0;

	for(uint32_t i = 0; i < jpeg->size; i++){
		if(jpeg->data[i] > PIXEL_OVEREXPOSURE_LIMIT) saturation_high_temp++;
	}
	return saturation_high_temp;
}


static uint32_t saturation_low(const JpegInformation *jpeg){

	uint32_t saturation_low_temp = 0;

	for(uint32_t i = 0; i < jpeg->size; i++){
		if(jpeg->data[i] < PIXEL_UNDEREXPOSURE_LIMIT) saturation_low_temp++;
	}
	return saturation_low_temp;
}

static ImageStatus image_suitable(
		const uint8_t mean,
		const float var,
		const uint32_t saturation_low_variable,
		const uint32_t saturationHigh_variable,
		const uint32_t size
		){

//-----Brightness exposure
	if(mean > MEAN_UPPER_THRESHOLD) return IMAGE_BRIGHTNESS_HIGH; // image is too exposed to light, return 0 e.g not suitable
	if(mean < MEAN_LOWER_THRESHOLD) return IMAGE_BRIGHTNESS_LOW; // image is too dark, not suitable

//----- Texture in the image

	if(var < VARIANCE_THRESHOLD) return IMAGE_TEXTURE_LOW; // not enough variation in pixels thus, image is probably usesless.

//------ are too many pixel's spoiled
	if (saturationHigh_variable > size * MAX_SATURATED_HIGH_RATIO) return IMAGE_SATURATION_HIGH;

	if (saturation_low_variable > size * MAX_SATURATED_LOW_RATIO) return IMAGE_SATURATION_LOW;

	return IMAGE_OK; // image is good


}


ImageStatus image_processing(
		const JpegInformation *jpeg,
		ImageQuality *image_info
		){



	image_info->mean_val = mean(jpeg);

	image_info->variance_val = variance(jpeg,image_info->mean_val);

	image_info->saturation_high_val = saturation_high(jpeg);

	image_info->saturation_low_val = saturation_low(jpeg);




	image_info->suitability_val = image_suitable(
			image_info->mean_val,
			image_info->variance_val,
			image_info->saturation_low_val,
			image_info->saturation_high_val,
			jpeg->size
			);




	return image_info->suitability_val;










}





